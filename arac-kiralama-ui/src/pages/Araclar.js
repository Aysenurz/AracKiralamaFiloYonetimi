...
export default function Araclar() {
  ...
  useEffect(() => {
    ...
    const url = `Araclar/GetByFilter?subeId=${subeId}&alis=${alis}&donus=${donus}`;
  ...
  return (
    <div className="min-h-screen bg-gray-100 px-10 py-10">
      ...
      <div className="bg-white p-5 rounded-xl shadow mb-10 max-w-3xl mx-auto">
        <p><strong>Kiralama Noktası:</strong> {subeId}</p>
        <p><strong>Alış Tarihi:</strong> {alis}</p>
        <p><strong>Dönüş Tarihi:</strong> {donus}</p>
      </div>

      {uniqAraclar.length === 0 ? (
        <p className="text-center text-gray-600">Bu kriterlere uygun araç bulunamadı.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {uniqAraclar.map((arac) => (
            <div key={arac.modelId} className="bg-white rounded-xl shadow p-4">
              <img
